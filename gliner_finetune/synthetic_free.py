...
def generate_data(source_json, num_calls, folder="synthetic_data", raw_filename="raw_responses.json", 
                      json_filename="parsed_responses.json", overwrite_file=False, model="mixtral-8x7b-32768"):
    
    # Load environment variables from .env file
    load_dotenv()
    api_key = os.getenv("grocq_api")
    if api_key is None:
        raise ValueError("API Key must be set in the environment variables.")
    
    client = Groq(api_key= api_key)  # Initialize the OpenAI client
...
def process_example(example):
    ...
    # Example usage
example_data = {
    "text": "Bipin Saha, Machine Learning Engineer, Graduated from Dept. of Electrical and Electronic Engineering, University of Rajshahi, and pursuing MSc in Robotics and Mechatronics Engineering, University of Dhaka",
    "person_name": ["Bipin Saha"],
    "email_address": ["bsaha88751@gmail.com"],
    "mobile_number": ["01727661854"],
    "designition": ["Machine Learning Engineer"],
    "university": ["University of Rajshahi", "University of Dhaka"],
    "degree": ["Robotics and Mechatronics Engineering", "Electrical and Electronic Engineering"]
}
...
num_calls = 1000  # Number of times the user wants to send the prompt
